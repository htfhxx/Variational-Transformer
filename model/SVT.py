...
from model.common_layer import EncoderLayer, DecoderLayer, VarDecoderLayer1, MultiHeadAttention, \
    PositionwiseFeedForward, LayerNorm, _gen_bias_mask, _gen_timing_signal, share_embedding, LabelSmoothing, NoamOpt, \
    _get_attn_subsequent_mask, _get_attn_self_mask, get_input_from_batch, get_output_from_batch, gaussian_kld, \
    SoftmaxOutputLayer
...
class VarDecoder(nn.Module):
    ...
    def forward(self, inputs, encoder_output, posterior, mask):
        ...
        # Run decoder
        y, _, _, attn_dist, _, means, log_vars, logits_probs = self.vardec((x, encoder_output, posterior, [],
                                                                            (mask_src, dec_mask, dec_mask_p),
                                                                            {"prior": [], "posterior": []},
                                                                            {"prior": [], "posterior": []}, []))
...
class CvaeTrans(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        pre_logit, attn_dist, mean, log_var, probs = self.decoder(self.embedding(dec_batch_shift) + meta.unsqueeze(1),
                                                                  encoder_outputs, True, (mask_src, mask_trg))
        ## compute output dist
        ...
        if config.model == "cvaetrs":
            ...
            for prob in probs:
                ...
                sbow.unsqueeze(2).repeat(1, 1, seq_len).masked_fill_(sbow_mask,
                                                                     config.PAD_idx)  # [batch, seq_len, seq_len]
    ...
    def train_n_batch(self, batchs, iter, train=True):
        ...
        for batch in batchs:
            ...
            pre_logit, attn_dist, mean, log_var, probs = self.decoder(
                self.embedding(dec_batch_shift) + meta.unsqueeze(1), encoder_outputs, True, (mask_src, mask_trg))
            ## compute output dist
            logit = self.generator(pre_logit, attn_dist, enc_batch_extend_vocab if config.pointer_gen else None,
                                   extra_zeros, attn_dist_db=None)
            ## loss: NNL if ptr else Cross entropy
    ...
    def decoder_greedy(self, batch, max_dec_step=50):
        ...
        for i in range(max_dec_step + 1):
            ...
            out, attn_dist, _, _, _ = self.decoder(self.embedding(ys) + meta.unsqueeze(1), encoder_outputs, False,
                                                   (mask_src, mask_trg))
...
class CvaeNAD(nn.Module):
    ...
    def __init__(self, vocab, emo_number, model_file_path=None, is_eval=False, load_optim=False):
        ...
        if config.num_var_layers > 0:
            self.decoder = VarDecoder2(config.emb_dim, hidden_size=config.hidden_dim, num_layers=config.hop,
                                       num_heads=config.heads,
                                       total_key_depth=config.depth, total_value_depth=config.depth,
                                       filter_size=config.filter)
        else:
            self.decoder = VarDecoder3(config.emb_dim, hidden_size=config.hidden_dim, num_layers=config.hop,
                                       num_heads=config.heads,
                                       total_key_depth=config.depth, total_value_depth=config.depth,
                                       filter_size=config.filter)
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        pre_logit, attn_dist, mean, log_var = self.decoder(meta, encoder_outputs, r_encoder_outputs,
    ...
    def decoder_greedy_po(self, batch, max_dec_step=50):
        ...
        meta_size = meta.size()
        meta = meta.repeat(1, 50).view(meta_size[0], 50, meta_size[1])
        out, attn_dist, mean, log_var = self.decoder(meta, encoder_outputs, r_encoder_outputs,
                                                     (mask_src, mask_res, mask_trg))
        prob = self.generator(out, attn_dist, enc_batch_extend_vocab, extra_zeros, attn_dist_db=None)
        _, batch_out = torch.max(prob, dim=1)
        batch_out = batch_out.data.cpu().numpy()
